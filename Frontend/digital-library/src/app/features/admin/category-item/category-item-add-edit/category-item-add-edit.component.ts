...
export class CategoryItemAddEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (categoryIdParam) {
      ...
      this._snackBar.open('No category id given.', 'OK', {duration:3000});
      this.goToCategories();
    }
    if (idParam) {
    ...
    if (this.categoryItemForm.valid && this.categoryId !== null) {
      ...
      if (this.isEditMode && this.categoryItemId !== null) {
        ...
        this.categoryItemService
          ...
          .subscribe({
            next: () => {
              this._snackBar.open('Category item edited succesfully.', 'OK', {duration:3000});
              this.goBack();
            },
            error: (error: HttpErrorResponse) => {
              if (error.status === 404) {
        ...
        this.categoryItemService.createCategoryItem(categoryItem).subscribe({
          next: () => {
            this._snackBar.open('Category item added succesfully.', 'OK', {duration:3000});
            this.goBack();
          },
          error: (error: HttpErrorResponse) => {
            if (error.status === 400) {
